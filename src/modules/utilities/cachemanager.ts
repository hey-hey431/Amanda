...
for (const key of Object.keys(permissionstable)) {
	// @ts-ignore
	const k: keyof typeof permissionstable = key
	if (key === "ALL") continue
	permissionstable["ALL"] |= permissionstable[k]
}

const channelManager = {
	get: async function(id: string, fetch = false, convert = true) {
		const d: import("raincache/dist/cache/ChannelCache") | null = await client.rain.cache.channel.get(id)
		if (d) {
	...
	fetch: async function(id: string) {
	...
	filter: async function(search: string, guild_id: string, limit = 10): Promise<Array<any>> {
		const payload: { id: string, name: string, limit: number, guild_id?: string } = {
			id: search,
			name: search,
			limit
		}
	...
	typeOf: async function(channel: { id: string }) {
		const chan = await channelManager.get(channel.id, true, false) as import("@amanda/discordtypings").ChannelData
		if (chan) {
	...
	getOverridesFor: async function(channel: { id: string }) {
	...
	permissionsFor: async function(channel: { id: string; guild_id: string }): Promise<{ allow: number; deny: number }> {
		const value = { allow: 0x00000000, deny: 0x00000000 }
	...
	hasPermissions: async function(channel: { id: string; guild_id: string }, permission: number | keyof typeof permissionstable, permissions?: { allow: number; deny: number }) {
		if (!channel.guild_id) return true
		if (!permissions) permissions = await channelManager.permissionsFor(channel)

		/** @type {number} */
		let toCheck: number
		if (typeof permission === "string" && permissionstable[permission]) toCheck = permissionstable[permission]
...
const userManager = {
	...
	filter: async function(search: string, limit = 10): Promise<Array<any>> {
	...
	parse: function(user: any) {
		return new Discord.User(user, client)
	}
}

const memberManager = {
	get: async function(id: string, guildID: string, fetch = false, convert = true) {
		...
		if (md && ud) {
			...
			if (fetch) {
				...
				if (fetched) {
					if (convert) return memberManager.parse(fetched)
					else return fetched
				} else return null
			} else return null
		}
	},
	fetch: async function(id: string, guildID: string) {
	...
	filter: async function(search: string, guild_id: string, limit = 10): Promise<Array<any>> {
		const payload: { nick: string, username: string, discriminator: string, id: string, tag: string, limit: number, guild_id?: string } = { nick: search, username: search, discriminator: search, id: search, tag: search, limit }
		if (guild_id) payload.guild_id = guild_id
		const ds = await passthrough.workers.cache.getData({ op: "FILTER_MEMBERS", params: payload })
		return ds
	},
	parse: function(member: any) {
		return new Discord.GuildMember(member, client)
	},
	permissionsFor: async function(userID: string, guildID: string) {
		const value = { allow: 0x00000000, deny: 0x00000000 }
		...
		const roles: Array<string> = clientmemdata.roles || []
...
const guildManager = {
	...
	parse: function(guild: any) {
		return new Discord.Guild(guild, client)
	},
	async getOverridesFor(id: string) {
		const value = { allow: 0x00000000, deny: 0x00000000 }
...
export const cacheManager = { validate, users: userManager, channels: channelManager, members: memberManager, guilds: guildManager }
