...
import common from "../commands/music/common"
...
let autoPayTimeout: NodeJS.Timeout
...
async function autoPayTimeoutFunction() {
	/** @type {Array<string>} */
	const donors: Array<string> = await utils.sql.all("SELECT * FROM Premium").then(rows => rows.map(r => r.userID))
...
utils.addTemporaryListener(client, "messageReactionAdd", path.basename(__filename), async (data) => {
	...
	// @ts-ignore
	const user: Discord.User = await utils.cacheManager.users.get(data.user_id, true, true)
...
async function manageReady() {
	...
	utils.sql.all("SELECT * FROM AccountPrefixes WHERE userID = ?", [client.user!.id]).then(result => {
		if (!result) {
			...
			const row = result.find(r => r.status)
			if (row) statusPrefix = row.prefix
			else statusPrefix = "&"
		}
	...
	if (firstStart) {
		...
		client.lavalink.once("ready", async () => {
			...
			const data: { queues: Array<any> } = await passthrough.nedb.queue.findOne({ _id: `QueueStore_${config.cluster_id}` })
			if (!data) return
			if (data.queues.length > 0) passthrough.queues.restore()
		})

		client.lavalink.on("error", (error) => {
			// @ts-ignore
		...
		utils.sql.all("SELECT * FROM RestartNotify WHERE botID = ?", [client.user!.id]).then(result => {
			result.forEach(async row => {
				// @ts-ignore
				const channel: Discord.TextChannel = await utils.cacheManager.channels.get(row.channelID, true, true)
