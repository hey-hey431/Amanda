...
export class TriviaGame extends Game {
	...
	lang: any
	answers: Array<{ correct: boolean, answer: string }> | undefined
	...
	async start() {
		...
		const wrongAnswers = this.data.incorrect_answers.map((a: any) => a.trim())
		this.answers = wrongAnswers
			.map((answer: any) => ({ correct: false, answer }))
		...
		// Answer Fields
		const answerFields: Array<Array<any>> = [[], []]
	...
	async end() {
		...
		const results = await Promise.all(winners.map(async w => {
			// @ts-ignore
			const result: { userID: string, winnings: number } = {}
			result.userID = w[0]
...
async function startGame(channel: import("thunderstorm").PartialChannel, options: { suffix?: string; msg?: import("thunderstorm").Message; category?: number; lang: import("@amanda/lang").Lang }) {
		...
		if (options.suffix.includes("categor")) {
			options.msg!.author.send(
				new Discord.MessageEmbed()
					...
					.setDescription(`${d.trivia_categories.map((c: any) => c.name).join("\n")}\n\n${options.lang.games.trivia.prompts.categorySelect}`)
			).then(() => {
			...
			let f = d.trivia_categories.filter((c: any) => c.name.toLowerCase().includes(options.suffix!.toLowerCase()))
			if (options.suffix.toLowerCase().endsWith("music")) f = d.trivia_categories.filter((c: any) => c.name == "Entertainment: Music")
	...
	await channel.sendTyping()
	// Get new game data
	const body: Array<{ response_code: number; results: Array<TriviaResponse> }> = await JSONHelper(`https://opentdb.com/api.php?amount=1${category ? `&category=${category}` : ""}`, channel, options.lang)
	if (!body[0]) return
	const data = body[1]
 ...
 */
function sweeper(difficulty: string, size: number): { text: string; size: number; bombs: number; error?: boolean } {
	let width = 8,
		bombs = 6,
	...
	rows.forEach((row, index) => {
		row.forEach((item, iindex) => {
			if (item == pieceBomb) {
				...
				const num = (it: any) => { return typeof it == "number" }
				const bmb = (it: any) => { return it == pieceBomb }
				const undef = (it: any) => { return it == undefined }
