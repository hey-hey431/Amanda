...
import songTypes from "./songtypes.js"
reloader.sync("./commands/music/songtypes.js", songTypes)

import queueFile from "./queue.js"
...
const subcommandsMap: Map<string, { voiceChannel?: "ask" | "required" | "provide"; queue?: "required"; code: (msg: import("thunderstorm").Message, args: Array<string>, _: ({ voiceChannel: import("thunderstorm").VoiceChannel; queue: import("./queue").Queue; lang: import("@amanda/lang").Lang })) => any }> = new Map([
	["play", {
		...
		code: async (msg, args, { voiceChannel, lang }) => {
			...
			// @ts-ignore
			const guild: Discord.Guild = await utils.cacheManager.guilds.get(msg.guild!.id, true, true)
			...
			if (match && match.type == "video" && match.id) {
				...
				if (linkedIndex == null || args[2]) {
					...
					const action = { ignore: "total", remove: "all", actionType: "js", actionData: (message: import("thunderstorm").Message, emoji: { name: string, id: string }) => {
						...
						if (choice == 0) {
							...
							tracks = tracks.slice(linkedIndex as number)
							common.inserters.fromDataArray(message.channel, voiceChannel, tracks, insert)
						} else if (choice == 2) {
							// choice == 2: play linked item only
							common.inserters.fromData(message.channel, voiceChannel, tracks[linkedIndex as number], insert)
						}
					} }
				...
				common.inserters.fromSoundCloudLink(msg.channel, voiceChannel, msg, insert, match.link as string, lang)
			} else if (match && match.type === "spotify") {
				common.inserters.fromSpotifyLink(msg.channel, voiceChannel, msg, insert, match.link as string, lang)
			} else if (match && match.type === "newgrounds") {
				common.inserters.fromNewgroundsLink(msg.channel, voiceChannel, msg, insert, match.link as string, lang)
			} else if (match && match.type === "external") {
				common.inserters.fromExternalLink(msg.channel, voiceChannel, msg, insert, match.link as string, lang)
			} else {
	...
	["youtube", {
		code: async (msg, args, { lang }) => {
			...
			// @ts-ignore
			const guild: Discord.Guild = await utils.cacheManager.guilds.get(msg.guild.id)
...
commands.assign([
	{
		...
		async process(msg, suffix, lang) {
			...
			function send(text: string, announce = true, throwFailed = false) {
	...
	},
	{
		...
		async process(msg, suffix, lang) {
			...
			const types: { [s: string]: Array<[string, number]> } = {
		...
		async process(msg, suffix, lang) {
			...
			if (["original", "deep", "chill", "classics"].includes(suffix)) { // valid station?
				...
				*/
				...
				function makeZWSP(length: number) {
	...
	},
	{
		...
		async process(msg, suffix, lang) {
			...
			const subcommandObject = subcommandsMap.get(key as string)
			...
			// @ts-ignore
			const subcommmandData: { lang: import("@amanda/lang").Lang, queue: import("./queue").Queue, voiceChannel: Discord.VoiceChannel } = {}
