		...
		async process(msg, suffix, lang) {
			...
			if (playlistName == "show") {
				...
				async function getAuthor(author: string) {
					// @ts-ignore
					const user: Discord.User = await utils.cacheManager.users.get(author, true, true)
					if (user) {
			...
			if (action.toLowerCase() === "bulk" || action.toLowerCase() === "bulkadd") {
				...
				utils.createMessageCollector({ channelID: msg.channel.id, userIDs: [msg.author.id] }, (msgg) => {
					if (!msgg) return msg.channel.send("Fuck. Something broke. Pls report")
				...
				// Resolve the content
				const result: { id: string; title: string; lengthSeconds: number } | null = await (async () => {
				...
				/** @type {Discord.User} */
				// @ts-ignore
				const user: Discord.User = await utils.cacheManager.users.get(playlistRow.author, true, true)
